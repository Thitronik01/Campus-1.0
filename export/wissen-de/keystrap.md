# KeyStrap - NFC-Zugangsmedium

Route: /de/keystrap | Stand: 2026-07-07T00:00:00.000Z | Sichtbarkeit: standard
Quelle: Thitronik Online/project-data/runtime/wiki/wiki-articles/de/keystrap.json

---
KeyStrap - NFC-Zugangsmedium

KeyStrap ist ein NFC-Zugangsmedium fuer das THITRONIK NFC Modul . Es dient als beruehrungsnaher Transponder zum Scharf-/Unscharfschalten und, bei passender safe.lock -Anbindung, zum Ver-/Entriegeln.

## Kompatibilitaet

Erforderlich ist ein montiertes und angelerntes NFC Modul .

Kompatible Alarmanlagen folgen dem NFC Modul: WiPro III , WiPro III safe.lock , WiPro easy und C.A.S. III.

Ver-/Entriegeln der Zentralverriegelung ist nur mit WiPro III safe.lock und geeigneter Fahrzeuganbindung moeglich.

Pro NFC Modul sind insgesamt bis zu 14 NFC-Medien speicherbar.

## Technische Daten

Parameter | Value | 
 Article no. | 105302 / 105464-105470 | 
 Kompatibilität | THITRONIK NFC Modul | 
 Frequency | 13,56 MHz | 
 NFC range | max. 15 mm | 
 Dimensions | 231,5 x 20 x 6,5 mm | 
 Gewicht | 18 g | 
 Handgelenkumfang | M ca. 216 mm / L ca. 232 mm | 

## Anwendungsfaelle

Armband fuer Sport, Strand, Campingplatz oder Wassersport.

Zugang ohne Fahrzeugschluessel, Karte oder Smartphone in der Tasche.

Sichtbares persoenliches Medium fuer Nutzer, die es am Handgelenk tragen wollen.

## Montage / Einrichtung

NFC Modul in den Anlernmodus bringen.

Zugangsmedium direkt an die markierte NFC-Flaeche halten.

Bestaetigung am NFC Modul abwarten.

Funktion am Fahrzeug testen: scharf/unscharf, bei WiPro III safe.lock zusaetzlich Ver-/Entriegeln.

## Grenzen & typische Fehler

KeyStrap ist kein eigenstaendiger Sender und funktioniert nur am NFC Modul.

Metallbedampfte, beheizbare oder dicke Scheiben koennen die NFC-Reichweite reduzieren.

Fremd-Tags koennen je nach Typ kopierbar sein; fuer sicherheitsrelevanten Betrieb THITRONIK Originalmedien verwenden.

Wird mit dem Originalschluessel verriegelt, koennen safe.lock-Fahrzeuglogiken den spaeteren Zugang ueber THITRONIK Zubehoer einschraenken.

## FAQ

Can KeyStrap control WiPro directly? 
No. The NFC Modul evaluates the medium and then sends the command to WiPro via 868,35 MHz.

Can it operate central locking? 
Yes, but only with WiPro III safe.lock and a suitable vehicle connection.

What is the key support check? 
First check whether the NFC Modul is paired correctly and whether the medium is reliably detected at the mounting position.

## Artikelnummern

Art.-Nr. | Variant | 
 105302 / 105464-105470 | KeyStrap | 

## Quellenhinweis

Technische Daten und Reichweiten stammen aus den offiziellen THITRONIK Produktseiten sowie aus der NFC Modul Dokumentation im lokalen Quellenbestand.

## Querverweise

NFC Modul 

Zugangsmedien & Bedienung 

WiPro III 

KeyCard 

KeyTag 

KeyStrap
